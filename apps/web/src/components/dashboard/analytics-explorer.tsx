...
import { Card, CardContent, CardHeader, CardTitle } from '@portal-sekolah/ui';
      ...
      <CardHeader className="flex flex-col gap-4 pb-2 md:flex-row md:items-center md:justify-between">
        <div className="text-left">
          <CardTitle className="flex items-center gap-2 text-lg">
            {mode === 'trend' ? <LineChart className="size-5 text-primary" /> : <BarChart3 className="size-5 text-primary" />}
            {title}
          </CardTitle>
            ...
            {data.map((point) => (
              ...
              >
                <div className="flex flex-1 items-end gap-1">
                  <div
                    className="w-full rounded-t-lg bg-primary transition-opacity group-hover:opacity-80"
                    style={{ height: `${Math.max(10, (point.value / maxValue) * 100)}%` }}
                  />
                  {mode === 'bar' && point.secondary ? (
                    <div
                      className="w-full rounded-t-lg bg-muted-foreground/50 transition-opacity group-hover:opacity-80"
        ...
        <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
          {data.slice(0, 4).map((point) => (
            <Link
              key={point.id}
              href={point.href}
              className="inline-flex h-9 items-center justify-between gap-3 rounded-xl border bg-background px-3 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground"
              onClick={() => setChartKey(point.id)}
            >
