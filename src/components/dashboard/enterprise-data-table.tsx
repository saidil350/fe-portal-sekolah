...
export function EnterpriseDataTable<T extends DashboardEntity>({
  ...
  const filteredData = React.useMemo(() => {
    const query = (externalSearchQuery !== undefined ? externalSearchQuery : globalFilter).trim().toLowerCase();
    if (!query) return data;
    return data.filter((item) =>
      Object.values(item).some((value) => String(value ?? '').toLowerCase().includes(query))
    );
  }, [data, globalFilter, externalSearchQuery]);
  ...
  React.useEffect(() => {
    setPageIndex(0);
  }, [globalFilter, externalSearchQuery]);
