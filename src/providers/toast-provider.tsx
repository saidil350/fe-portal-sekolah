    ...
    <ToastContext.Provider value={{ toast, toasts, dismiss }}>
      ...
      <div className="fixed bottom-4 right-4 z-50 flex w-full max-w-md flex-col gap-2 pointer-events-none">
        {toasts.map((toastItem) => (
          <div
            key={toastItem.id}
            className={`pointer-events-auto flex items-start justify-between gap-4 rounded-lg border p-4 shadow-sm ${
          ...
          >
            ...
            <button type="button" onClick={() => dismiss(toastItem.id)} className="text-xs font-bold opacity-70 hover:opacity-100">
              ×
            </button>
          </div>
        ))}
